  ...
  build(node: HTMLElement) {
    return import('raw-loader!./soccer.csv').then((content: any) => {
      ...
      return builder(parsed.data)
        ...
        .column(buildCategoricalColumn('current_league').width(180).label('Current League'))
        ...
        .column(buildNumberColumn('games', [0, NaN]).asArray(labels).width(120).custom('type', 'matrix').custom('stratifications', strats))
        .column(buildNumberColumn('goals', [0, NaN]).asArray(labels).width(120).custom('type', 'matrix').custom('stratifications', strats))
        .column(buildNumberColumn('minutes', [0, NaN]).asArray(labels).width(120).custom('type', 'matrix').custom('stratifications', strats))
        .column(buildNumberColumn('assists', [0, NaN]).asArray(labels).width(120).custom('type', 'matrix').custom('stratifications', strats))
        .deriveColors()
        .aggregationStrategy('group+top+item')
